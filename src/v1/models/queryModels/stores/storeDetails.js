...
exports.StoreDetailsData = async (params, userId, org_id, next) => {
    logger.info("*** Starting %s of %s ***", getName().functionName, getName().fileName);
    try {
        params = params.data;
        let dataEx = await db.StoreDetails.findOne({ where: { name: params.name, org_id: org_id } });
        if (!dataEx) {
...
exports.storeDetailsUpdateData = async (params, userId, org_id, next) => {
    ...
    try {
        ...
        if (dataExist != null) {
            let duplicate_checking = await db.StoreDetails.findOne({ where: { name: params.name} });
